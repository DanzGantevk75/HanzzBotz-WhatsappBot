...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      try {
        ...
        if (user) {
          ...
          if (!isNumber(user.premiumTime)) user.premiumTime = 0    
            ...
            if (!isNumber(user.haa)) user.haa = 0
     ...
     if (!isNumber(user.goldtiket)) user.goldtiket = 0
        
        if (!isNumber(user.banteng)) user.banteng = 0
     if (!isNumber(user.harimau)) user.harimau = 0
     if (!isNumber(user.bata)) user.bata = 0
     ...
     if (!isNumber(user.kentang)) kentang = 0
     if (!isNumber(user.padi)) user.padi = 0
     if (!isNumber(user.nyawa)) user.nyawa = 3
     if (!isNumber(user.anvil)) user.anvil = 0
     if (!isNumber(user.cinta)) user.cinta = 0
     if (!isNumber(user.kardus)) user.kardus = 0
     
 
            if (!isNumber(user.lastadventure)) user.lastadventure = 0
            if (!isNumber(user.us)) user.us = 0
            if (!isNumber(user.lastberkebun)) user.lastberkebun = 0
            if (!isNumber(user.lastkencan)) user.lastkencan = 0
            if (!isNumber(user.lastfishing)) user.lastfishing = 0
            if (!isNumber(user.lastdungeon)) user.lastdungeon = 0
            if (!isNumber(user.lastdungeon)) user.lastkumpuljamur = 0
            ...
            if (!isNumber(user.crafting)) user.crafting = 0
            ...
            if (!isNumber(user.level)) user.level = 0
            if (!isNumber(user.istri)) user.istri = -1
            if (!('kareshi' in user)) user.kareshi = ''
...
let settings = global.DATABASE.data.settings
      ...
      let isPrems = isROwner || DATABASE.data.users[m.sender].premium
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (m.chat in global.DATABASE._data.chats || m.sender in global.DATABASE._data.users) {
            ...
            if (!['unbanuser.js', 'inv.js', 'link.js', 'creator.js', 'profile.js'].includes(name) && user && user.Banneduser && !isROwner) {
              if (!opts['msgifbanned']) m.reply(`*ANDA TERBANNED*\nKarena Anda Melanggar Rules

Hubungi: 
${global.creator.map((v, i) => '*Owner ' + (i + 1) + ':* wa.me/' + v).join('\n') + '\n\n' + global.mods.map((v, i) => '*Moderator ' + (i + 1) + ':* wa.me/' + v).join('\n')}
`)
              return
            }
          }
          ...
          if (!isPrems && plugin.limit && global.DATABASE._data.users[m.sender].limit < plugin.limit * 1) {
            this.send3Button(m.chat, `Limit/Tiket Anda Habis, Silahkan Beli Dengan Klik Button Dibawah, Atau Jadilah User Premium Agar Limit Anda Unlimited, Jika Berminat Klik Button Dibawah`, wm, 'Beli Semua', '.buyall', 'Beli 1', '.buy 1', 'Premium', '.premium', m)
        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              let wel = `https://www6.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=800&scaleHeight=500&fontsize=100&fillTextType=1&fillTextPattern=Warning!&text=Welcome`
              let lea = `https://www6.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=800&scaleHeight=500&fontsize=100&fillTextType=1&fillTextPattern=Warning!&text=GoodBye`

              this.sendButtonImg(jid, action === 'add' ? wel : lea, text, 'ʜᴀɴᴢᴢ ʙᴏᴛᴢ ʙy ʜᴀɴᴢᴢ-ᴋᴜɴ', action === 'add' ? 'Hai Welcome👋👋' : 'Bye Bye👋👋', 'Welcome By Hanzz-kun')
            }
          }
        }
        break
...
*.off delete*
`.trim(), wm, 'OFF ANTIDELETE', '.off delete', m.message, {
      contextInfo: {
    ...
    async onCall(json) {
        ...
        await this.sendMessage(from, '「 Reject Call 」\nMaaf Bot Tidak Bisa Menerima Panggilan!\n\nSorry Bot Can\'t Receive Calls!\n\nTelpon/call = Block', MessageType.extendedText)
...
╰────── `
  ...
  if (msgg) return conn.sendButton(m.chat, msgg, global.wm, '⬡ 𝘿𝘼𝙁𝙏𝘼𝙍', `.daftar ${namae}.18`, m, { contextInfo: { mentionedJid: [syappa] }})

    let msggg = {
rowner: `*[❗] Creator Only* 
Perintah ini Hanya Dapat Digunakan Oleh Creator`, 
owner: `*[❗] Owner Only* 
Perintah ini Hanya Dapat Digunakan Oleh Owner`,
mods: `*[❗] Moderator Only* 
Perintah ini Hanya Dapat Digunakan Oleh Moderator`,
premium: `*[❗] Premium User Only* 
Beli Premium Dengan Ketik .premium`,
group: `*[❗] Group Only* 
Perintah Ini Hanya Dapat Digunakan Di Group*`,
private: `*[❗] Private Only* 
Perintah ini Hanya Dapat Digunakan Pada Chat Pribadi`,
...
Perintah ini Hanya Dapat Digunakan Saat Bot Menjadi Admin`,
  ...
  if (msggg) return conn.sendButton(m.chat, msggg, global.wm, 'OK', `SIP`, m, { contextInfo: { mentionedJid: [syappa] }})
}
