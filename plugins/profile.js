...
let handler = async (m, { conn, text }) => {
  ...
  if (typeof DATABASE.data.users[who] == 'undefined') {
    DATABASE.data.users[who] = {
      exp: 0,
      limit: 10,
      registered: false,
  ...
  try {
    ...
    let { name, limit, exp, registered, regTime, age, banned, level, premium, premiumTime, role } = global.DATABASE.data.users[who]
...
Level ${level}
`.trim()
    await conn.sendButtonImg(m.chat, pp, str, wm, '⋮☰Menu', '.menu', m)
  }
}
